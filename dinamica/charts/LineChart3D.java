...
/**
 * Chart plugin for Line charts 3D with multiseries support
 * Last update: 24/july/2005
 * @author Martin Cordova (dinamica@martincordova.com)
 */
public class LineChart3D extends AbstractChartPlugin
{
		...
		throws Throwable
	{
		...
		while (data.next())
		{
			...
			{
				Double value = new Double(String.valueOf(data.getValue(dataCols[i])));
				if (value==null) 
					value = new Double(0);

				chartdata.addValue(value.doubleValue(), series[i], label);
